...
Changing the configured history location migrates the active file immediately. Other tracked files migrate the next time they are opened. A common folder stores every revision bucket beneath its hidden `.npphistory` child so plugin data does not clutter the selected folder. Older common-folder buckets created directly inside the selected folder are migrated into that hidden child at startup or when settings are applied; source folders are removed only after verified success. Opening or activating a file also discovers matching older history in its adjacent `.npphistory` folder. Those revisions are safely merged into the common location without overwriting existing history; comments and timestamps are preserved. INFO logging records the file, source, destination and counts; failures are logged as WARNING and leave the source available for retry.
