...
- Corrected common history storage to use a hidden `.npphistory` child beneath the selected folder. Existing catalogue-owned revision buckets from the former visible layout are migrated immediately at startup or after settings changes, with comments, timestamps and revisions preserved; successful source removal is logged at INFO and failures retain the source and log a WARNING.
- Added DEBUG history-location checks for every reconciled file, including whether revision history is enabled, the exact matching exclusion pattern (if any), configured location, adjacent folder, number of legacy history folders found, and migration outcome.
...
- Corrected toolbar/pane repaint hazards: no repeated full-toolbar invalidation for unchanged states, no plugin-driven autosizing of the native rebar toolbar, and final parent/child repaint after pane layout. Added focused live checks for native toolbar bounds, wrapping, uniform sizes and non-overlapping pane controls. Replacement review build awaits installed-environment confirmation.
- Source-only: enriched all temporary action messages with filename/revision context and consistent casing/punctuation. Comparison reports when its view opens. Each status event now uses a shared reporter to emit matching INFO, WARNING or CRITICAL log text, with full paths/diagnostics in log detail. Critical is included by all existing enabled severity thresholds without changing saved settings. Added exact-wording/log-pair tests and updated native log assertions; not compiled or executed.
- Source-only: brief NppHistory action feedback in the native status bar for captures, refreshes, comparison closure, restores, revision deletions/comment edits, saves, settings and update-check results. Messages expire after five seconds, preserve the other native fields and yield to native text changes. Regression cases prepared; compilation and runtime verification remain deferred.
- Source-only: toolbar and hotkey changes now apply on OK without restarting. All toolbar commands are registered once and selectively shown/hidden, including after toolbar rebuilds. Plugin-owned, UI-thread-only shortcuts replace old bindings atomically, refresh menu labels, avoid modal/menu/AltGr input and key-repeat actions, and invalidate queued commands after settings or document changes. Cancel leaves active choices unchanged. Compilation and native validation are deferred at the user's request.
- Source-only refinements (not yet compiled): replaced the fixed Plugins column with independent tab-bar context-menu choices, renamed Pane to History Pane, locked History off inside its own pane, and audited all Settings tooltips. Labels/tab headings no longer show hints; comment-editor/About buttons and comparison text controls gain missing explanations. Both right-click menus share the submenu/separators setting.
