...
- Adds separate automatic-saving and revision-history wildcard exclusion lists, with distinct crossed-out Auto Save and Revision History tab icons and a wrapping **File Excluded in Settings** pane status. Each icon appears only while its feature is enabled. Affected tabs expand individually along their text direction, horizontally or vertically, so icons fit after the filename and before pin/close; ordinary sizes, public filenames, buffer identities, shared padding and ordering remain unchanged.
