...
void runLoggerTests(TestContext& context)
{
    ...
    logger.write(LogLevel::debug, L"History location check",
        L"File: C:\\Notes\\NppHistory.log | Revision History: Enabled | History Exclusion: Matched (*.log) | Configured Location: Common folder | Adjacent Folder: C:\\Notes\\.npphistory | History Folders Found: 0 | Result: No adjacent history found");
    ...
    context.expect(debugText.find(L"[DEBUG] History location check | File: C:\\Notes\\NppHistory.log | Revision History: Enabled | History Exclusion: Matched (*.log) | Configured Location: Common folder")
        != std::wstring::npos && debugText.find(L"Result: No adjacent history found") != std::wstring::npos,
        "PluginLogger records the matching exclusion in an enriched history-location check");
